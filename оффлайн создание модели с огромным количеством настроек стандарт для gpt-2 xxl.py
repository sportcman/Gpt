...
model_config = GPT2Config(
    ...
    n_layer=48, # Количество слоев (layers) в трансформере. Большее количество слоев может помочь модели улавливать более сложные зависимости в данных, но требует больше вычислительных ресурсов
    n_head=64, # Количество "голов" (heads) в механизме внимания (attention mechanism). Большее количество голов позволяет модели фокусироваться на разных аспектах данных одновременно и улучшает ее способность к обучению зависимостей
    n_embd=4096, # Размерность вектора эмбеддинга для каждого токена. Определяет сколько информации может содержаться в представлении каждого слова
    intermediate_size=16384, # Размер скрытого слоя промежуточного представления. Определяет размерность скрытого слоя между слоем внимания и слоем линейного преобразования
    hidden_size=4096, # Размер скрытого состояния модели. Определяет размерность скрытого состояния каждого слоя трансформера
    max_position_embeddings=4096, # Максимальная длина последовательности, которую модель может обрабатывать. В итоге и выводить не более задоной цыфры. Если последовательность превышает это значение, то она будет обрезана или заполнена специальными токенами
    ...
    layer_norm_epsilon=1e-7, # Эпсилон для Layer normalization, используемый для стабилизации обучения
