...
class ScriptManager(QMainWindow):
    ...
    def new_script(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(self, "Создать новый скрипт", self.directory, "Python files (*.pyw)")
        if filename:
    ...
    def load_script(self):
        ...
        # Загружаем скрипт
        filename, _ = QFileDialog.getOpenFileName(self, "Загрузить скрипт", self.directory, "Python files (*.pyw)")
        if filename:
